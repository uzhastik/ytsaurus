...
Read `task_description.json` first to understand the component and scope, then read **every** `.diff` file in order.
...
6. **Skip trivial changes** like whitespace-only fixes, import reordering, or comment typos — unless they are part of a larger meaningful change.

7. **Focus on user impact.** Describe what changed from the end user's perspective, not internal implementation details.
...
15. **Exclude cross-component changes.** Some commits touch files within the component's path but actually belong to a different component's domain. These must be excluded from the release notes. Determine the true audience of each change by analyzing what the code actually does, not just where the file lives. Common examples to **exclude**:
    ...
    - **Build system changes** (`ya.make`, `CMakeLists.txt`) — unless they affect the distributed package.

    **Rule of thumb:** If a change does not affect the public API, CLI behavior, or user-visible behavior of the distributed package (e.g., `ytsaurus-client` on PyPI), it should be excluded. Ask yourself: "Would a user of `pip install ytsaurus-client` notice or care about this change?" If not, skip it.
